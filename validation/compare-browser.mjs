...
const rows=filter?JSON.parse(readFileSync(resolve(here,'chromium-report.json'),'utf8')).results.filter(r=>!r.fixture.startsWith(filter)):[];
try {
for(const f of readdirSync(resolve(root,'fixtures')).filter(f=>f.endsWith('.svg')&&(!filter||f.startsWith(filter)))){
 ...
 writeFileSync(resolve(here,name+'-chromium.png'),PNG.sync.write(png));
 ...
 writeFileSync(resolve(here,name+'-chromium-diff.ppm'),`P3\n64 64\n255\n${diff.join(' ')}\n`);
}
writeFileSync(resolve(here,'chromium-report.json'),JSON.stringify({reference:await browser.version(),referenceScale:SCALE,results:rows},null,2)+'\n');
console.table(rows);
