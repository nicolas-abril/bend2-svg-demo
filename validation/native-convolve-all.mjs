...
const hash=f=>createHash('sha256').update(readFileSync(resolve(root,f))).digest('hex'),sourceFiles=['svg.bend','state.bend','render.bend','fonts/NotoSans-Regular.ttf','fonts/NotoSans-Bold.ttf','fonts/NotoSans-Italic.ttf','fonts/NotoSans-BoldItalic.ttf'],sourceSHA256=Object.fromEntries(sourceFiles.map(f=>[f,hash(f)])),binarySHA256=hash('build/render');
for(const fixture of readdirSync(resolve(root,'fixtures')).filter(f=>f.endsWith('.svg')).sort()){
 ...
 const got=spawnSync(resolve(root,'build/render'),['--gpu','off'],{env:{...process.env,SVG_FONTS:resolve(root,'fonts'),SVG_INPUT:resolve(root,'fixtures',fixture),SVG_WIDTH:'64',SVG_HEIGHT:'64',SVG_AA:'8'},encoding:'utf8',maxBuffer:32*1024*1024});if(got.status!==0)throw Error(got.stderr||got.stdout);
...
writeFileSync(resolve(here,'convolve-native-all-report.json'),JSON.stringify({passed,check:'Saved C backend versus the JavaScript-produced 64x64 AA8 matrices for every fixture',sourceSHA256,binarySHA256,results},null,2)+'\n');if(!passed)throw Error('Backend matrix differences: '+results.filter(r=>!r.passed).map(r=>r.fixture).join(', '));console.log(`All ${results.length} C/JavaScript matrices match exactly`);
