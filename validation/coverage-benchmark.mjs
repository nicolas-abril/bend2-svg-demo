...
function run(binary,fixture){const t=performance.now(),r=spawnSync(binary,['--gpu','off'],{env:{...process.env,SVG_FONTS:resolve(root,'fonts'),SVG_INPUT:resolve(root,'fixtures',fixture),SVG_WIDTH:'64',SVG_HEIGHT:'64',SVG_AA:'8'},encoding:'utf8',maxBuffer:32*1024*1024});if(r.status!==0)throw Error(r.stderr||r.stdout);return{seconds:(performance.now()-t)/1000,output:r.stdout};}
