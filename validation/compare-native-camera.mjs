...
const root='/private/tmp/bend-svg-work',v=root+'/validation',candidate=root+'/camera-investigation';
const png=PNG.sync.read(readFileSync(v+'/native-camera-preview.png'));
const ppm=readFileSync(v+'/camera-wide-expected.ppm','utf8').trim().split(/\s+/).slice(4).map(Number);
