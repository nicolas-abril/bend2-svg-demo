...
writeFileSync(resolve(here,'native-all-report.json'),JSON.stringify({passed,check:'Saved C backend versus the JavaScript-produced 64x64 AA8 matrices for every fixture',sourceSHA256,binarySHA256,results},null,2)+'\n');if(!passed)throw Error('Backend matrix differences: '+results.filter(r=>!r.passed).map(r=>r.fixture).join(', '));console.log(`All ${results.length} C/JavaScript matrices match exactly`);
